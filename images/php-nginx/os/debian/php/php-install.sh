...
main()
{
    ...
    case "${install_type}" in
        ...
        source) compile_from_source ;;
        *) trigger_error "Invalid PHP_INSTALL_TYPE '${install_type}'; Must be 'repo' or 'source'" ;;
    esac

    # sanity check
    php-fpm --version | grep -qE "^PHP [0-9]+[.][0-9]+[.][0-9]+" || trigger_error "php installation failed"
}
...
install_from_repo()
{
    ...
    # fix bin paths (pain in the ass with this changing every version)
    local bin_name

    # php-fpm
    bin_name=$(ls -1 /usr/sbin | grep php-fpm)
    test -z "${bin_name}" && trigger_error "expected a versioned php-fpm binary in /usr/sbin"
    ln -s "/usr/sbin/${bin_name}" /usr/sbin/php-fpm

    # php
    rm -f /usr/bin/php # I know I could just leave that alone, but I prefer the bin dir cleaner
    rm -f /usr/bin/php.default
    bin_name=$(ls -1 /usr/bin | grep php)
    test -z "${bin_name}" && trigger_error "expected a versioned php binary in /usr/bin"
    ln -s "/usr/bin/${bin_name}" /usr/sbin/php
